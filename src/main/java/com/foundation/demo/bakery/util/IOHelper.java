...
//		/* test ton tai file*/
        ...
        try {
            try (FileOutputStream fos = new FileOutputStream(path, true); PrintWriter pw = new PrintWriter(fos)) {
                if (!checkExists) {
                    pw.println(
    ...
    public static List<Product> productReader(String path) {
        ...
        try (Reader reader = new FileReader(path);) {
            ...
            CsvToBean<Product> csvToBean = new CsvToBeanBuilder<Product>(reader).withMappingStrategy(strategy)
                    .withIgnoreLeadingWhiteSpace(true).build();

            csvToBean.parse().forEach(obj -> {
                products.add(obj);
            });
